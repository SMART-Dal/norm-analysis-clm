...
"""PyTorch RoBERTa model. 
    Modified to calculate vector norms based on
    Kobayashi's implementation: https://github.com/gorokoba560/norm-analysis-of-transformer/blob/master/emnlp2020/transformers/src/transformers/modeling_bert.py
"""
...
class RobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Original implementation by Goro Kobayashi
        if output_norms:
...
# Original class implementation by Goro Kobayashi:

class RobertaNormOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.num_attention_heads = config.num_attention_heads
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
...
class RobertaAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        # Original implementation by Goro Kobayashi
        self.norm = RobertaNormOutput(config)
    ...
    def forward(
        ...
        past_key_value: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
        output_attentions: Optional[bool] = False,
        output_norms: Optional[bool] = False,  # Original implementation by Goro Kobayashi
    ) -> Tuple[torch.Tensor]:
        ...
        if output_norms:
            ...
            outputs = (attention_output, self_outputs[1],) + norms_outputs
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(
            ...
            output_norms=output_norms # Original implementation by Goro Kobayashi
        )
...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        all_norms = () # Original implementation by Goro Kobayashi
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(
                    ...
                    past_key_value,
                    output_attentions,
                    output_norms, # Original implementation by Goro Kobayashi
                )
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        output_norms: Optional[bool] = None, # Original by Goro Kobayashi
